...
## Layout/Design
Overall solid design work across the site. The only major issues I have are regarding different size screens as you'll see below. Also, it seems super nitpicky but your front page of a site should always be in the top level folder. This is because when a server loads, if you don't have custom routing (and sometimes even if you do) it defaults to whatever html file is called `index.html` in the top level folder for the site. At this point its accepted best practice even WITH custom rouuting.
